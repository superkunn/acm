...
int dx[4]={0,1,0,-1};
int dy[4]={1,0,-1,0};
int n,m,q,a,b,d,p[MAXN],x[MAXN],y[MAXN],ans[MAXM];
char op[5];
struct node1{
    int a,b,d,c;
    node1(){}
...
struct node2{
    int a,b,t,id;
    node2(){}
    node2 (int a,int b,int t,int id):a(a),b(b),t(t),id(id){}
}no2[MAXM];
...
int work(){
    ...
    rep(i,1,q){
        ...
        int aa=findp(kk.a);
        int bb=findp(kk.b);
        if(aa!=bb)ans[kk.id]=-1;
        else ans[kk.id]=fun(x[kk.a],y[kk.a],x[kk.b],y[kk.b]);
    }
    rep(i,1,q)printf("%d\n",ans[i]);
    return 0;
}
